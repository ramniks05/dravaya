...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { merchant_reference_id } = req.body as { merchant_reference_id: string }
    const resp = await fetch(`${API_BASE_URL}/api/v1/payout/transactionStatus`, {
      method: 'POST',
      headers: {
        'api-Key': API_KEY,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ merchant_reference_id })
    })
